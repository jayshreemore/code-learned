...
if($Country!='')
{
    if($Country=='India' || $Country=='india' || $Country=='IN')   // India
    {
        ...
        if($p_lenght>0 && $p_lenght==10)
        {
            ...
            echo "<script type=text/javascript>alert('SMS has been sent Successfully on $phone'); window.location='Send_Mail_Group_admin.php'</script>";
        }
        else
        {
            echo "<script type=text/javascript>alert('Sorry,Invalid Phone No.'); window.location='Send_Mail_Group_admin.php'</script>";
        }
    }
    elseif($Country=='US' || $Country=='USA')                // for USA
    {
        ...
        echo "<script type=text/javascript>alert('SMS has been sent Successfully on $number'); window.location='Send_Mail_Group_admin.php'</script>";
        $date=(new \DateTime())->format('Y-m-d H:i:s');
...
{
    echo "<script type=text/javascript>alert('Sorry,Unable to send message without Country name'); window.location='Send_Mail_Group_admin.php'</script>";
}
?>