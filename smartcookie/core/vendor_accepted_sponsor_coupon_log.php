...
//tbl_selected_vendor_coupons
 ...
 $q= mysql_query("select sv.coupon_id,sp.id, sv.timestamp, sv.for_points, sv.code, sv.entity_id, sv.user_id, sp.Sponser_product from tbl_selected_vendor_coupons sv 
 left join tbl_sponsored sp on sp.id=sv.coupon_id and sp.sponsor_id=sv.sponsor_id
where sv.used_flag='used' and sv.sponsor_id='$_SESSION[id]' order by sv.id desc")or die(mysql_query()); 
...
<h2 class="panel-title"><b>Accepted Sponsor Coupons Log </b><span class="badge"><?php echo mysql_num_rows($q); ?></span></h2>
</div>
...
<td><?php echo $res['Sponser_product']; ?></td>
<td><?php echo $res['for_points']; ?></td>
