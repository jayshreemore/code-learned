...
<div style="height:10px;"></div>
		...
		<div style="height:50px; background-color:#FFFFFF; border:1px solid #CCCCCC;box-shadow: 0px 1px 3px 1px #C3C3C4;" align="left">
        <h2 style="padding-left:20px; margin-top:2px;color:#666">Green points to <?php echo $dynamic_teacher?> for distribution</h2>
        </div>
		...
		<?php
			...
			$teachers_id=array();
               ...
               $arrs = mysql_query("SELECT t.id,t.t_id, t_complete_name, tc_balance_point,  sum(sc_point) as sum, sc_entites_id FROM tbl_teacher t
						LEFT JOIN tbl_student_point st ON t.t_id = st.sc_teacher_id
						WHERE (`t_emp_type_pid`='133' or `t_emp_type_pid`='132') AND t.school_id = '$school_id' and t.tc_balance_point!=0 group by t_complete_name ORDER BY t.t_complete_name ASC ");
