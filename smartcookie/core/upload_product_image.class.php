...
class Uploader{
	public function photoUpload(){
		$path = 'images/uploaded_product_image/';
		...
		if((($photo_type == 'image/jpeg') || ($photo_type == 'image/gif')   ||
		   ...
		   ($photo_size < 2000000) && in_array($post_ext,$file_ext)){
			...
			if(file_exists($path.$photo_name)){
				...
				$photo_name = 'SC_'.md5($new_name[0]).'.'.$new_name[1];
...
/* 	        $th_image = imagecreatetruecolor($th_w, $th_h);
	        ...
	        // generate thumbnail
	        imagejpeg($th_image, $newdir.'thumbnail_'.$img, 100); */
  ...
  public function dumpPhoto($thumb,$dir,$f){
