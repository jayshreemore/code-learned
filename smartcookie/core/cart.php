...
$school_id=$_SESSION['school_id'];
	...
	$p = mysql_query("SELECT available_points FROM cart WHERE `user_id`='$user_id' and `entity_id`='2' and coupon_id is null");  
	$pt=mysql_fetch_array($p);
	$maxpt=$pt[0];
		
	$cartinfo =mysql_query("SELECT * FROM cart WHERE `entity_id` ='2' and `user_id`='$user_id'");
 ...
 */
		...
		$q=mysql_query("UPDATE `tbl_student_reward` sr INNER JOIN tbl_student s ON sr.sc_stud_id=s.std_PRN SET `sc_total_point`='$pts_green', `yellow_points`='$pts_yellow', `purple_points`='$pts_purple' WHERE s.id='$id' and s.std_PRN='$rid' ");
			...
			$p=mysql_query("UPDATE `cart` SET `available_points`='$pts' WHERE `user_id`='$user_id' and `entity_id`='2' and `coupon_id` is null");
		}


			if($q and $p){
				
			$q = mysql_query("SELECT * FROM cart WHERE `user_id`='$user_id' and `entity_id`='2' and `coupon_id`!=''");
				$sr=1;
...
$insert_selected_vendor_coupons=mysql_query("INSERT INTO tbl_selected_vendor_coupons (id, entity_id, user_id,	coupon_id, for_points, timestamp, code,	used_flag, sponsor_id, valid_until, school_id) VALUES(NULL,\"2\",\"$user_id\",\"$cid\",\"$ppp\",CURRENT_TIMESTAMP,\"$code\",\"$used_flag\",\"$sp_id\",\"$valid_until\",\"$school_id\")");
				$delete_cart=mysql_query("DELETE FROM `cart` WHERE `user_id`=$user_id and `entity_id`=2 and `coupon_id`!=''");
				 $sr++;
				 } 
...
<div class="panel panel-default">
  <div class="panel-heading"><b>My Coupons</b></div>
	...
	<thead>
	<tr ><th>#</th><th>Coupon ID</th><th>Company Logo</th><th>Company Name</th><th>Product Name</th><th>Address</th><th>Points</th><th>Delete</th></tr></thead><tbody>
   <?php
	$sr=1;$tot=0;
	while($result = mysql_fetch_array($cartinfo )){
		...
		<?php if(file_exists($logo)){ ?><td><img src="<?php echo $logo;?>" style="height:83px; width:120px;" /></td><?php } else{ ?><td><img src="image/newlogo1.png" style="height:83px; width:120px;" /></td><?php } ?>
			...
			<td><?php echo $address; ?></td><td><?php echo $pts;?></td><td><a href="cart.php?delitem=<?php echo $cart_row_id; ?>"><h4><input type="button" name="del" value="Delete"></h4></a></td></tr>
			<?php $tot=$tot+$pts; $sr++;
		}
	} 
	?>
	</tbody>
	<tr><td>Remaining Points</td><td><?php echo $rm=$maxpt-$tot;?></td><td></td><td></td><td></td><td></td><td><b><?php echo $tot;?></b></td><td></td></tr>
	</table>
