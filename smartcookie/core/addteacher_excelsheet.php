		...
		$file_type1=$file_t1[1];
		 ...
		 $storagename = "Importdata/" . $_FILES["file"]["name"];
			...
			$inputFileName = $storagename; 
				...
				$mobile=str_replace("'","", trim($allDataInSheet[$i]["D"]));   // mobile
				$dept_name=str_replace("'","", trim($allDataInSheet[$i]["E"]));  // Dept. Name
				$gender=str_replace("'","", trim($allDataInSheet[$i]["F"]));  // Gender
				$email_id=str_replace("'","", trim($allDataInSheet[$i]["G"]));  // Email ID
				$country=str_replace("'","", trim($allDataInSheet[$i]["H"])); //country
				$address=str_replace("'","", trim($allDataInSheet[$i]["I"])); //t_address
				$dob=str_replace("'","", trim($allDataInSheet[$i]["J"]));  // T_DOB
				$email_id1=str_replace("'","", trim($allDataInSheet[$i]["K"])); //t_internal Email
				$landline=str_replace("'","", trim($allDataInSheet[$i]["L"]));//t_landline
				$date_appointment=str_replace("'","", trim($allDataInSheet[$i]["M"]));// t_date_ppointment
				$employee_type_id=str_replace("'","", trim($allDataInSheet[$i]["N"]));//Employee type PID
				
				
				$teacher_name=$arr[$i]["C"];
				$first_name=explode(" ",$teacher_name);
				 $name_length=count($first_name);
				...
				$sql1=mysql_query("select * from tbl_raw_teacher where t_school_id='$school_id' and t_id='$t_id' and batch_id='$batch_id'");
				...
				$count1=mysql_num_rows($sql1);
										...
										if($count1==0)
										{
											...
											$err_flag="Correct";
												$results=$smartcookiefunctions->teacher_raw_register($t_id,$teacher_name,$t_firstname,$t_middlename,$t_lastname,$school_name,$school_id,$dept_name,$address,$country,$dob,$gender,$email_id,$email_id1,$password,$t_date,$mobile,$landline,$batch_id,$storagename,$file_type1,$DataCount,$date1,$uploaded_by,$err_flag,$date_appointment,$employee_type_id);
										}
				...
				$sql_query1=mysql_query("select * from tbl_raw_teacher where t_school_id='$school_id' and t_id='$t_id' and batch_id='$batch_id' and error_records='Correct' ");
				$scan=$_POST['scan'];
				...
				$teacher_id=$result_query['t_id'];
				$teacher_complete_name=$result_query['t_complete_name'];
				$teacher_first_name=$result_query['t_name'];
				$teacher_middle_name=$result_query['t_middlename'];
				$teacher_last_name=$result_query['t_lastname'];
				$t_school_name=$result_query['t_school_name'];
				$t_school_id=$result_query['t_school_id'];
				$t_dept=$result_query['t_dept'];
				$t_address=$result_query['t_address'];
				$t_country=$result_query['t_country'];
				$t_dob=$result_query['t_dob'];
				$t_gender=$result_query['t_gender'];
				$t_email=$result_query['t_email'];
				$t_internal_email=$result_query['t_internal_email'];
				$t_password=$result_query['t_password'];
				$t_date=$result_query['t_date'];
				$t_phone=$result_query['t_phone'];
				$t_landline=$result_query['t_landline'];
				$batch_id=$result_query['batch_id'];
				
					$t_date_of_appointment=$result_query['t_date_of_appointment'];
				$t_emp_type_pid=$result_query['t_emp_type_pid'];
				...
				$sql3=mysql_query("select * from tbl_teacher where school_id='$t_school_id' and t_id='$teacher_id'");
				...
				if($count3==0)
				{
															...
															
								{
									...
									$err_flag="Correct";
						$results=$smartcookiefunctions->teacher_register($teacher_id,$teacher_complete_name,$teacher_first_name,$teacher_middle_name,$teacher_last_name,$t_school_name,$t_school_id,$t_dept,$t_address,$t_country,$t_dob,$t_gender,$t_email,$t_internal_email,$t_password,$t_date,$t_phone,$t_landline,$batch_id,$err_flag,$t_date_of_appointment,$t_emp_type_pid);
