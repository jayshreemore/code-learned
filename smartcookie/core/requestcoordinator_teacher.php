...
window.location = "studentcoordnatorrequest.php?t_id="+t_id;
}
...
<body style="background-color:#FFFFFF">
 ...
 <div class="row">
<h1 align="center">Request for Assigning Coordinator</h1>
 </div>
   ...
   if(isset($_POST['submit']))
	{
		...
		$sql=mysql_query( "select distinct t.t_id,t.id,t.t_name,t.t_complete_name,t.t_lastname,t.t_pc,t.t_email,t.t_phone,sm.subjectName from tbl_teacher t join tbl_student_subject_master sm  on sm.teacher_id=t.t_id JOIN tbl_academic_Year a ON sm.AcademicYear = a.Year
	...
	$i=1;
	 ...
	 <tr> <td  style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:12px;"><?php echo $i;?>
                ...
                <?php }else {?> <img src="image/avatar_2x.png"  style="border:1px solid #CCCCCC; width:70px;height:70px;" class="preview" alt="Responsive image"/> <?php }?>
                          ...
                          <form method="post">
                            <input type="hidden" name="teacherid" value="<?php echo $test['id']?>">
                         
                         <?php 
						 $t_id=$test['id'];
						
						  $query_coordintor=mysql_query(" select id from tbl_request where stud_id1='$id' and stud_id2='$t_id' and entitity_id='112'");
						 $result_coordintor=mysql_num_rows($query_coordintor);
					
						 if($result_coordintor==0)
						 {
						  ?>
                            <td> <a class="txt-button" ><input type="button" value="Send Request" id="request_button"  name="request_button" class="btn btn-success" onClick="openwindow(<?php echo $test['id'];?>);"></a></td>
                            
                            <?php }
							
							else
							{?>
								   <td> <a class="txt-button" ><input type="button" value="Request Sent" class="btn btn-success" disabled> </a></td>
							<?php }
							?>
...
$query=mysql_query("select id,t_pc,t_name,t_middlename,t_lastname,t_complete_name,t_email,t_phone,t_id from tbl_teacher where t_id='$teacher_id' and school_id='$school_id'");
