...
if($value['t_complete_name']!='')
{
	...
	while(count($arr)>$i)
	{
		$teacher_name=ucfirst(strtolower($arr[$i]))." ";
				...
				$reasonofreward="";
							...
							$point_date = date('d/m/Y');
								...
								if(substr($studentpointlist_id,0,8)=="subject-")
								{
								 $reason_id=substr($studentpointlist_id,8);
								  ...
								  $row_activity=mysql_query("SELECT distinct(subjectName) FROM `tbl_student_subject_master` WHERE subjcet_code ='$reason_id'");
								...
								$row_activity=mysql_query(" SELECT sc_list FROM `tbl_studentpointslist` WHERE `sc_id ` = '$reason_id'");
								$value_activity=mysql_fetch_array($row_activity);
					...
					mysql_query("insert into tbl_student_point(sc_stud_id,sc_entites_id,sc_teacher_id,sc_studentpointlist_id,sc_point,marks,point_date,sc_status,method,activity_type) values('$stud_id','$entites_id','$teacher_id','$reason_id','$point','$marks','$point_date','N','$method','$activity_type')");
							...
							// Message to be sent
								...
								$row_student=mysql_query("select * from tbl_student where std_PRN='$std_PRN'");
								$value_student=mysql_fetch_array($row_student);
							$Gcm_id=$value_student['Gcm_id'];
...
<body style="background: none repeat scroll 0% 0% #E5E5E5;text-shadow: none;">
      <div class="" style="padding-top:20px; padding-left:20px;">
	  <div class="col-md-12">
		<div class="col-md-3 ">
...
WHERE sm.student_id = '$std_PRN' AND sm.school_id = '$school_id' AND Enable = '1' AND sm.teacher_ID='$teacher_id'");
                            ...
                            <input type="radio"   value="<?php echo 'subject-'.$sub_id;?>" name="activity" id="<?php echo 'subject-'.$sub_id;?>" onclick = "MyAlert(this.value)"/> <span style="font-size:12px;">&nbsp;&nbsp;<?php echo  ucfirst(strtolower($row['subjectName']));?><br/></span>
                             <?php
