...
if(isset($_POST['submit']))
{
	...
	if(mysql_num_rows($get_stud_info)==0)
   {
         ...
         $log = "insert into `tbl_student_point` (sc_stud_id,sc_entites_id,sc_studentpointlist_id,sc_point,point_date,school_id) values('$stud_id','','$reason','$points','$date','$school_id')" ;
        $que= mysql_query($log);
		 ...
		 $updatecount=mysql_affected_rows();
		  ...
		  '$reason','$points','$date','$school_id','activity','assigned by school admin','green_Point')" ;
        $que= mysql_query($log);
			 if($updatecount>0)
			 {		 
