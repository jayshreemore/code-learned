...
<script>
    ...
    function confirmEmail(Status,school_id,email) {
        //alert(email);return false;
        if (Status == 'Send_Email') {
            var answer = confirm("Are you sure,do you want to resend Email");
            if (answer) {
               window.location = "SendEmail_Teacher.php?email="+email+"&school_id="+school_id+"";
            }
            else {
            }
        } else {
    ...
    function confirmSMS(phone,school_id,Status,country) {
        if(Status=='Send_SMS'){
            ...
            if (answer) {
                window.location = "SendSMS_Teacher.php?phone="+phone+"&school_id="+school_id+"&Status="+Status+"&country="+country;
            }
            else {
...
<div style="bgcolor:#CCCCCC">
    <div class="container" style="padding:30px;width:1600px">
            ...
            <div style="background-color:#F8F8F8 ;">
                <div class="row">
                    ...
                    <div class="col-md-6 " align="center">
                        <h2>Send SMS/EMAIL to Teachers</h2>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-4">
                        <div class="dropdown1">
                            <button class="btn btn-default dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-expanded="true">Send SMS/ Email to Teachers<span class="caret"></span></button>
                    ...
                    <div class="row" style="padding:10px;">
                        <div class="col-md-12  " id="no-more-tables">
                            <table class="table-bordered  table-condensed cf" id="example" width="100%;">
                                ...
                                <?php
                                $i = 1;
                                $arr = mysql_query("select id,t_id,t_complete_name,t_phone,t_email,t_internal_email,batch_id,send_unsend_status,email_status,school_id,t_country,sms_time_log,email_time_log from tbl_teacher where school_id='$sc_id' and t_emp_type_pid in ('133','134') and error_records like 'Correct' order by id"); ?>
                                <?php while ($row = mysql_fetch_array($arr)) {
                                    ...
                                    <tr style="color:#808080;" class="active">
                                        ...
                                        <td data-title="Phone" style="width:10%;">
                                            <a onclick="confirmSMS('<?php echo $row['t_phone']; ?>','<?php echo $row['school_id']; ?>','<?php echo $row['send_unsend_status'];?>','<?php echo $row['t_country']; ?>');"><img src="Images/S.png" ></a>
                                            <a onclick="confirmEmail('<?php echo $row['email_status'];?>','<?php echo $row['school_id']; ?>','<?php if($row['t_email']=="")
										{
											echo $row['t_internal_email'];
										}
										else
