...
<script>
    ...
    function confirmEmail(Status,school_id,email,name) {
        ...
        if (Status == 'Send_Email') {
            ...
            if (answer) {
                window.location = "SendEmail_Student.php?email="+email+"&school_id="+school_id+"&name="+name;
            }
            else {
            }
        } else {
    ...
    function confirmSMS(phone,school_id,Status,country) {
        if(Status=='Send_SMS'){
            ...
            if (answer) {
                window.location = "SendSMS_Student.php?phone="+phone+"&school_id="+school_id+"&Status="+Status+"&country="+country;
            }
            else {
            }
        }else{
	...
	function confirmSMSToAll(school_id) {
        ...
        //if(Status=='Send_SMS'){
            ...
            if (answer) {
                window.location = "SendSMS_toAll_student.php?school_id="+school_id;
            }
            else {
            }
        //}else{
            ...
            <div style="background-color:#F8F8F8 ;">
				...
				<a onclick="confirmSMSToAll('<?php echo $sc_id ?>');"><img src="Images/Sms.png"></a>
                    ...
                    <div class="col-md-6 " align="center">
                        <h2>&nbsp&nbsp&nbsp&nbspSend SMS/EMAIL to Student's</h2>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-12  " id="no-more-tables">
                        ...
                        <table class="table-bordered  table-condensed cf" style="border: 1px solid #ccc;" id="example" width="100%;">
                            ...
                            <tr style="background-color:#428BCA">
                                ...
                                <th style="width:20%;"><b>Student PRN</b></th>
                                ...
                                <th style="width:20%;">Send SMS/Email</th>
                                </th>
                            </tr>
                            </thead>
                            <tbody>
                            <?php
                            $i = 1;
                            $arr = mysql_query("select id,std_PRN,std_complete_name,std_phone,std_branch,std_email,batch_id,send_unsend_status,email_status,school_id,std_country,sms_time_log,email_time_log from tbl_student where school_id='$sc_id'  order by batch_id"); ?>
                            <?php while ($row = mysql_fetch_array($arr)) {
                                ...
                                <tr style="color:#808080;" class="active">
                                    ...
                                    <td data-title="Phone" style="width:10%;">
                                     <a onclick="confirmSMS( '<?php echo $row['std_phone']; ?>','<?php echo $row['school_id']; ?>','<?php echo $row['send_unsend_status'];?>','<?php echo $row['std_country']; ?>');"><img src="Images/S.png"></a>
                                     <a onclick="confirmEmail('<?php echo $row['email_status'];?>','<?php echo $row['school_id']; ?>','<?php echo $row['std_email']; ?>','<?php echo $row['std_complete_name']; ?>');" ><img src="Images/E.png"></a>
                                    </td>
                                </tr>
                                <?php $i++;?>
                            <?php } ?>
