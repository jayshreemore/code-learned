...
if(!isset($_SESSION['id']))
	 ...
	 $query = mysql_query("select sc_total_point,yellow_points from tbl_student_reward where sc_stud_id = '$std_PRN'");
	 $st_points = mysql_fetch_array($query);
	
	$reward_points=$st_points['sc_total_point'];
	$yellow_points=$st_points['yellow_points'];
	...
	if($reward_points >= $points)
	{
		  ...
		  $test=mysql_query("update tbl_student_reward set sc_total_point='$final_points' where sc_stud_id='$std_PRN'");
		 
		 $arr=mysql_query("select * from tbl_student_reward where sc_stud_id='$student_id' ");
		 $arr1=mysql_fetch_array($arr);
		 
		  
		  $sc_final_point=$arr1['yellow_points']+$points;
		$sql1=mysql_query("update tbl_student_reward set yellow_points='$sc_final_point' where sc_stud_id='$student_id'");
			$sql2=mysql_query("update tbl_request set flag='Y' where stud_id1='$student_id' and stud_id2='$std_PRN' and entitity_id='105' and flag='N' and reason like '$reason'");
			
		$sql3=mysql_query("insert into tbl_student_point(sc_entites_id,sc_point,sc_teacher_id,sc_stud_id,reason,point_date) values('105','$points','$std_PRN','$student_id','$reason','$accept_date')");
		
		$report="Request successfully accepted";
	
header('Location: requestlist.php?report='.$report); 
	...
	$sql2=mysql_query("update tbl_request set flag='P' where stud_id1='$student_id' and stud_id2='$std_PRN' and entitity_id='105' and flag='N' and reason like '$reason'");
...
<?php 
 ...
 { 
  $query1 = mysql_query("select sc_total_point from tbl_student_reward where sc_stud_id = '$std_PRN'");
	 $st_points1 = mysql_fetch_array($query1);
	
	$reward_points1=$st_points['sc_total_point'];
  ...
  <?php
	while($post = mysql_fetch_assoc($arr1))
	{
	 $st_id=$post['stud_id'];
	 	...
	 	$sql=mysql_query("select std_name,std_Father_name,std_lastname,std_complete_name,std_img_path from tbl_student where std_PRN='$st_id'");
...
<div class="col-md-8"  style="padding:20px; border:1px solid #CCCCCC;box-shadow: 0px 1px 3px 1px #C3C3C4;">	
         <form method="post">    
            ...
            <?php if($value['std_img_path'] != ''){?>
              ...
              <div class="col-md-2"><input type="hidden" name="reason" id="reason" value="<?php echo $post['reason']?>" /><?php echo $post['reason']?></div> 
 ...
 <a href="requestlist.php"> Pending Requests</a>
 </div>
 </div>
