...
@require 'coupon.inc.php';
/* if($_SESSION['entity']==2){
	require 'header_teacher.inc.php';
} */
include_once 'up_cart.php'; 
$id=$_SESSION['id'];
...
if(!isset($_COOKIE['lat2']) and !isset($_COOKIE['lon2'])){
	$data= new data();
...
if(!empty($lat1) and !empty($lon1) or( !empty($lat2) or !empty($lon2))){	
...
if($dist_acceptable){
	...
	if($sp_website!=""){ ?><a href='http://<?php echo $sp_website; ?>' target='_blank' ><strong><?php echo $sp_website; ?></a></strong> 
						   ...
						   <input type="submit" name="select" value="Select" class="getcoubtn" /></p>
						   </form>
