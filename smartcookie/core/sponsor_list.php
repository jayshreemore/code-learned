...
if (isset($_POST['search'])) {
   ...
   // $sql="SELECT sp.id,sp.sp_company,sp.sp_address,sp.sp_email,sp.sp_phone,sp.sp_website,sp.sp_date,sp.amount,sp.sales_person_id,c.category from tbl_sponsorer sp  join categories c on sp.v_category=c.id  where sp.sales_person_id='$person_id'  and sp.sp_date between '$from_date' and '$to_date' ";
		...
		$sql = "SELECT * from tbl_sponsorer sp where sp.sales_person_id='$person_id'  and sp.sp_date between '$from_date' and '$to_date'";
    }
	if ($from_date != '' && $to_date != '' && $person_id != '' && $from_date ==$to_date) {
   // $sql="SELECT sp.id,sp.sp_company,sp.sp_address,sp.sp_email,sp.sp_phone,sp.sp_website,sp.sp_date,sp.amount,sp.sales_person_id,c.category from tbl_sponsorer sp  join categories c on sp.v_category=c.id  where sp.sales_person_id='$person_id'  and sp.sp_date between '$from_date' and '$to_date' ";

		$sql = "SELECT * from tbl_sponsorer sp where sp.sales_person_id='$person_id'  and sp.sp_date like '%$from_date%'";
    }
    ...
    if ($from_date != '' && $to_date != '' && $person_id == '' && $from_date !=$to_date) {
		...
		$sql = "SELECT * from tbl_sponsorer sp where sp.sp_date between '$from_date' and '$to_date'";
    }
	if ($from_date != '' && $to_date != '' && $person_id == '' && $from_date ==$to_date) {
			//$sql="SELECT sp.id,sp.sp_company,sp.sp_address,sp.sp_email,sp.sp_phone,sp.sp_website,sp.sp_date,sp.amount,sp.sales_person_id,c.category from tbl_sponsorer sp join categories c on sp.v_category=c.id where sp.sp_date between '$from_date' and '$to_date' ";

		$sql = "SELECT * from tbl_sponsorer sp where sp.sp_date like '%$from_date%'";
    }
    ...
    if (($from_date == '' || $to_date == '') && $person_id != '') {
		...
		$sql = "SELECT * from tbl_sponsorer sp  where sp.sales_person_id='$person_id'";
    }
  ...
  /*  if (($from_date == $to_date) && $person_id == '' && $from_date != '') {
	...
	if ($from_date == '' && $to_date == '' && $person_id != '' && $category != '' && $category != 'select') {
		//$sql="SELECT sp.id,sp.sp_company,sp.sp_address,sp.sp_email,sp.sp_phone,sp.sp_website,sp.sp_date,sp.amount,sp.sales_person_id,c.category from tbl_sponsorer sp  join categories c on sp.v_category=c.id  where sp.sp_date = '$from_date'";
	...
	if ($from_date != '' && $to_date != '' && $person_id == '' && $category != '' && $category != 'Select'  && $from_date==$to_date) {
	   ...
	   $sql = "SELECT * from tbl_sponsorer sp  where  sp.v_category='$category' and sp.sp_date like '%$from_date%'";
    }
    if ($from_date == "" && $to_date == "" && $person_id == "") {
		...
		$sql = "SELECT * from tbl_sponsorer sp";
    }


    if ($category != '' && $category != 'select'  && $from_date == '' && $to_date == '' && $person_id == '') {
...
//    die;
	...
	$sql = "SELECT * from tbl_sponsorer sp order by sp.id desc";
...
<div align="center">
    ...
    <div style="width:100%;">
        <div style="padding-top:50px;">
            <h2 style="padding-left:20px; margin-top:2px;;color:white;background-color:#694489;padding-top:10px;padding-bottom:10px">List of Sponsors</h2>
        </div>
        <div id="no-more-tables" style="padding-top:20px;">
            <table id="example" class="col-md-12 table-bordered ">
                <thead>
                <tr style="background-color:#694489; color:white; height:30px;">
                    <th>Sr. No.</th>
                    <th>Reg.Date</th>
					<!--<th> Country</th>-->
					<th> Sponsor Image</th>
					<th> Shop Image</th>
				<!--	<th> Lat-Long</th>
					<th> Sales Person Lat-Long</th>
					<th> Calculated Lat-Long</th>-->
                    <th>MemberId/Sponsor Name</th>
					<th>Company Name</th>
                ...
                <tr class='sortable' >
					...
					<?php if($result['sponaor_img_path']!=''){ ?>
						<td data-title="sponsor image"><img style='width:40%' src="<?php echo $index_url."/core/".$result['sponaor_img_path']; ?>"/></td>
					<?php  }else {?>
						<td data-title="sponsor image">NA</td>
					<?php } ?>

				<!--	<td data-title="latlong"><?php echo $result['lat']."</br>".$result['lon']; ?></td>
					<td data-title="latlong"><?php echo $result['sales_p_lat']."</br>".$result['sales_p_lon']; ?></td>
					<td data-title="callatlong"><?php echo $result['calculated_lat']."</br>".$result['calculated_lon']; ?></td>-->
					<td onclick="window.location.href = 'http://<?php echo $server_name; ?>/core/sponsor_deatils.php?id=<?php echo $result['id'] ?>';"
					data-title="Sponsor Name"><?php echo $result['id']."</br>".$result['sp_name']; ?></td>
					 <td data-title="Company Name"><?php echo ucwords($result['sp_company']); ?></td>
                    <td data-title="Address"><?php echo ucwords($result['sp_address']); ?></td>
					...
					?></td>
                    <td data-title="Email ID"><?php echo $result['sp_email']."</br>"; echo $result['sp_phone'];?></td>
                  <!--  <td data-title="amount"><?php if ($result['amount'] == "" or $result['amount'] == 0) {
					...
					<td data-title="v_responce_status"><?php echo $result['v_responce_status']."</br>".$result['v_status']; ?></td>
					<td data-title="source"><?php
					if($result['source'])
					{
						echo $result['source'];
					}
					...
					?></td>
					<td><?php echo $result['current_marketing']; ?></td>
					<td><?php echo $result['discount']; ?></td>
					<td><?php echo $result['digital_marketing']; ?></td>
					<td data-title="comment"><?php echo $result['comment']; ?></td>
					<td data-title="calback_date_time"><?php echo $result['calback_date_time']; ?></td>
					<td data-title="platform_source"><?php echo $result['platform_source']."</br>".$result['app_version']; ?></td>-->
