<?php
 ...
 //echo "select * from tbl_teacher where t_email = '".$_SESSION['username']."' and t_password = '".$_SESSION['password']."'";die;
$id= $_SESSION['id'];
...
$report3="";
	...
	$arr= mysql_query("update tbl_teacher set t_complete_name='$t_name',t_qualification='$quelification',t_dob='$date',t_age='$age',t_gender='$t_gender' where id ='$id'");
...
/*echo "update tbl_teacher set t_dept='$Department',t_exprience='$Experience',t_date_of_appointment='$Appointment',t_emp_type_pid='$EmployeeID' where t_id ='$t_id'";
	die;*/
	$arr= mysql_query("update tbl_teacher set t_dept='$Department',t_exprience='$Experience',t_emp_type_pid='$EmployeeID' where id ='$id'");
...
//-----------------------------------------------------------ContactDetails---------------------------------------------------------------------
		 ...
		 {
		    $Pss=$_POST['Pss'];
		    $cntrycode=$_POST['ccode'];
		    $cntryname=$_POST['cname'];
	...
	$arr= mysql_query("update tbl_teacher set t_password='$Pss',t_country='$cntryname',CountryCode='$cntrycode',t_phone='$MobileNo',t_landline='$Landline',t_email='$Email',t_internal_email='$InternalEmail',t_address='$Address',t_permanent_pincode='$Pincode' where id ='$id'");
...
//-----------------------------------------------------------My Class---------------------------------------------------------------------
	...
	$arr= mysql_query("update tbl_teacher set t_class='$class' where id ='$id'");
...
<p style="font-size:24px; color:#0080FF;">
                			<?php if(  $value['t_pc']==""){?><img id="imgprvw" name="img" src='image/avatar_2x.png' width="120" height="100" class="preview" /><?php }else{ ?><img src='<?php echo $value['t_pc'] ?>' width="120" height="120" class="preview" /><?php }?></p>
 ...
 function valid_contact()
    {
		...
		var countrycodecode=document.getElementById("ccode").value;
			if(countrycodecode=="")
			{

				document.getElementById('errorcc').innerHTML='Please Enter Country Code';
		...
		var countryname=document.getElementById("cname").value;
			if(countryname=="")
			{
				...
				document.getElementById('errorcname').innerHTML='Please Enter Country Name';
...
document.getElementById('errorpoints1').innerHTML='<input type="button" class="btn btn-primary pull-right text-center" name="Edit" value="Edit" onclick="myFunction2()">';
	...
	document.getElementById('ccode').innerHTML='<input type="text" size="27" id="ccode" name="ccode" value="<?php echo $value['CountryCode'];?>"/>';
	document.getElementById('cname').innerHTML='<input type="text" size="27" id="cname" name="cname" value="<?php echo $value['t_country'];?>"/>';
...
function myFunction5()
	...
	document.getElementById("ccode").innerHTML='<?php echo $value['CountryCode'];?>';
	document.getElementById("cname").innerHTML='<?php echo $value['t_country'];?>';
...
<table class = "table" style="border:1px solid #ccc">
	...
	<tbody>
		...
		<tr>
			...
			<td id="errorqua" style="color:red;"></td>
...
<!---------------------------------------------------------------------Work Place------------------------------------------------------------------------------------->
	...
	<thead>
		<tr>
			<th> <?php echo  $school_type.' Name';?></th>
	...
	<tbody>
		...
		<tr>
			<td><b> <?php echo $school_teacher.' ID';?> Id</b></td>
  ...
  <!---------------------------------------------------------------------Contact Details------------------------------------------------------------------------------------->
	...
	<tbody>
		<tr>
			...
			<td id="ccode"><?php echo $value['CountryCode'];?></td>
		</tr>
			<td id="errorcc" style="color:red;"></td>

		<tr>
			<td><b>Country Name</b></td>
			<td id="cname"><?php echo $value['t_country'];?></td>
		</tr>
			<td id="errorcname" style="color:red;"></td>
		<tr>
