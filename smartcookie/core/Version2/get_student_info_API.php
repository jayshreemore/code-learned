...
$obj = json_decode($json);
   ...
   if($entity_type=='Teachers')
   {
      if(preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,})$/",trim($user_type)))
                            ...
                            else if(preg_match("/^[a-zA-Z ]+$/",$user_type))
                            {

                                  $condition = "t_name LIKE '".$user_type."%' or t_lastname LIKE '".$user_type."%'";
                            }
                        	else
                        	{
   ...
   else
   {
        ...
        else if(preg_match("/^[a-zA-Z ]+$/",trim($user_type)))
        {

            $condition = "std_name LIKE '".$user_type."%' or std_lastname LIKE '".$user_type."%'";
        }
      	else
      	{
...
include 'conn.php';
    ...
    if(!empty($user_type) && !empty($school_id) && !empty($entity_type))
	{
        ...
        {
		    $query="SELECT s.*,sr.sc_total_point FROM `tbl_student` s left join tbl_student_reward sr on s.`std_PRN`=sr.sc_stud_id
             and s.`school_id`=sr.school_id WHERE $condition and s.`school_id`='$school_id'";
       }
