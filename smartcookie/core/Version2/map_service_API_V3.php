  ...
  function get_location_details($entity_type,$range,$lat1,$long1,$range_type,$loc_type,$place_name,$city,$input,$format)
  {
            ...
            if($entity_type=='SCHOOL')
            {
              if($loc_type=='CURRENT')
              {
                ...
                $limit=10;
                $sql="select * from tbl_school where school_latitude!='' and school_longitude!=''";
 			    $arr = mysql_query($sql);
        				 ...
        				 $lon2=$test['school_longitude'];
                          $distance=calculate_distance($lat1,$long1,$lat2,$lon2,$range_type);
                           $kilometers = $distance * 1.609344;
        			    	if($kilometers<=$range)
        				    {
								 $teacher = mysql_query("SELECT count(id) as total FROM tbl_teacher where school_id='".$test['school_mnemonic']."'");
							 $teacher_count = mysql_fetch_array($teacher);
							// $posts['teacher_count'] = $teacher_count['total'];
							 $test['teacher_count']=$teacher_count['total'];
							 
							 
							 $student = mysql_query("SELECT count(id) as total FROM tbl_student where school_id='".$test['school_mnemonic']."'");
							 $student_count = mysql_fetch_array($student);
							// $posts['student_count'] = $student_count['total'];
							$test['student_count'] = $student_count['total'];
