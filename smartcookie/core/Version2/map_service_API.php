 ...
 if(!empty($loc_type))
  ...
  function get_location_details($entity_type,$range,$lat1,$long1,$range_type,$loc_type,$place_name,$city,$input,$format)
  {
        ...
        else if($entity_type=='SPONSOR')
        {
            ...
            /* if($input==0)
            {
                $count= mysql_query("select count(*) as total from tbl_sponsorer where (`lat`!='' and `lon`!='') and (v_status!='Inactive' or v_status is null) and sp_city like '$city%'");
                $count_result =mysql_fetch_array($count);
                $count1=$count_result['total'];
            } */

              if($loc_type=='CURRENT')
              {
                $sql="select id,sp_name,sp_address,sp_city,sp_country,sp_email,sp_phone,lat,lon,v_category,sp_img_path,v_status from tbl_sponsorer where (`lat`!='' and `lon`!='') and (v_status!='Inactive' or v_status is null) and sp_city like '$city%' limit $limit offset $input";
            ...
            else if($loc_type=='CUSTOM')
            {
                    ...
                    /*  if($input==0)
                    {
                        $count= mysql_query("select count(*) as total from tbl_sponsorer where (`lat`!='' and `lon`!='') and (v_status!='Inactive' or v_status                                                  is null)");
                        $count_result =mysql_fetch_array($count);
                        $count1=$count_result['total'];
                    } */
                     // $sql="SELECT * FROM `tbl_sponsorer` where `lat`!='' and `lon`!='' and `sp_city` like '$place_name%'";
                    $sql="select id,sp_name,sp_address,sp_city,sp_country,sp_email,sp_phone,lat,lon,v_category,sp_img_path,v_status from tbl_sponsorer where (`lat`!='' and `lon`!='') and (v_status!='Inactive' or v_status is null) limit $limit offset $input";
 			            $arr = mysql_query($sql);
                    if(mysql_num_rows($arr)>=1)
                    {
        		    	while($test = mysql_fetch_assoc($arr))
           ...
           }else
              {
                        ...
                        $postvalue['responseStatus']=204;
        				$postvalue['responseMessage']="Invalid Entity type";
        				$postvalue['posts']=null;
                        display($postvalue,$posts,$format);
              }
   }
