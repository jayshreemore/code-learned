...
if ($cat_id != "" or ($dist != "" and $lat1 != "" and $lon1 != "")) {
	...
	if ($last_id != 0 AND $last_id != "") {
		$sql = "SELECT spd.validity, spd.id, sp.sp_img_path, sp.sp_company, spd.Sponser_type, spd.Sponser_product, sp.lat,sp.lon, spd.points_per_product, spd.sponsered_date, spd.sponsor_id, spd.product_image, spd.valid_until, cat.category, spd.product_price, spd.discount, spd.buy, spd.get, spd.saving, spd.offer_description, spd.total_coupons, spd.priority, spd.coupon_code_ifunique, spd.currency, spd.daily_counter, spd.daily_limit
	...
	if (mysql_num_rows($arr) >= 1) {
		while ($post = mysql_fetch_assoc($arr)) {
			...
			$start_check      = $td->datediffr($td, $sponsered_date);
			$start_check      = (int)$start_check;
			$validUntil_check = $td->datediffr($td, $valid_until);
