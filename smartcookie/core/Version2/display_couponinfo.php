...
$sp_id=$obj->{'sp_id'};
		...
		function imageurl($value,$type='',$img=''){
			...
			$tlogoUrl=@get_headers('http://$server_name/'.$value);
			$slogoUrl=@get_headers('http://$server_name/'.$value);
			if($logoUrl[0] == 'HTTP/1.1 200 OK' && $value!='new-product.jpg' && $value!='' ){
				...
				$logoexist='http://$server_name/core/'.$value;
	...
	if( $cp_id != "" )
		{
 			...
 			$arr = mysql_query("select c.cp_code, cp_stud_id,s.std_complete_name,s.std_name,s.std_lastname,s.school_id,s.std_Father_name,s.std_school_name,s.std_img_path,amount,c.status,cp_gen_date,c.validity from tbl_coupons c join tbl_student s on c.Stud_Member_Id=s.id where cp_code=\"$cp_id\"");
			...
			if(mysql_num_rows($arr)>=1){
				...
				while($post = mysql_fetch_assoc($arr)) {
					...
					if($post['cp_stud_id']!='' and $post['school_id']!='')
					{
						...
						$student_info_json = file_get_contents("http://$server_name/core/Version2/getstuddetail_info.php?User_Type=PRN&User_Name=$std_prn&school_id=$school_id");
					}
					elseif($post['Stud_Member_Id']!='')
					{
						...
						$student_info_json = file_get_contents("http://$server_name/core/Version2/getstuddetail_info.php?User_Type=MemberId&User_Name=$stud_member_id");
					}
					...
					$std_img_path=$student_info['std_img_path'];
						...
						if ($std_img_path==''){
                            ...
                            $image="http://$server_name/core/".$std_img_path;
						}
				...
				while($post = mysql_fetch_assoc($arr2)) {
					...
					if($std_img_path==''){
						$image=imageurl($post['t_pc'],'avatar','');
					}
					else{
                        $image="http://$server_name/core/".$std_img_path;
					}
