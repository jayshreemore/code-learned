<?php
	...
	if($count == 1)
	{
		...
		$result = mysql_query("SELECT twillioId FROM twillioresponse where number='$number' and AnsweredBy = 'SMS' and  `callStatus` = '' and app_date between '$app_date' and '$curr_date' order by twillioId desc limit 1");
	}
	else
	{
	 	$response = strtoupper($message[1]);
	 	$reminderid = $message[0];
		$result = mysql_query("SELECT twillioId FROM twillioresponse where number='$number' and AnsweredBy = 'SMS' and me_id = '$reminderid' and  `callStatus` = '' and app_date between '$app_date' and '$curr_date' order by twillioId desc limit 1");
	}
