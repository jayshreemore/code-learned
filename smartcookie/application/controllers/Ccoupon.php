		...
		if($this->form_validation->run()){	
			...
			if($this->input->post('iscurrent')=='current'){
				...
				//$lat=$this->input->post('lat');
					...
					$details = json_decode(file_get_contents("http://api.ipinfodb.com/v3/ip-city/?key=$_API&ip=$ip&format=json"));
			...
			$this->coupons->suggest_new_sponsor($this->entity, $this->id, $lat, $lon,$school_id);
